  ...
  return (
    <ReportWrapper title={`Ledger: ${d?.ledger_name ?? id}`} fromDate={fromDate} toDate={toDate} onFromChange={setFromDate} onToChange={setToDate} onExportExcel={() => {}} onExportPdf={() => {}} onPrint={() => window.print()}>
      <div className="rounded-xl border border-slate-200 dark:border-navy-100/20 overflow-hidden">
        <div className="bg-slate-100 dark:bg-navy-100/20 px-4 py-2 text-slate-500 dark:text-slate-400">Opening balance: <AmountDisplay amount={d?.opening_balance} /></div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-200 dark:border-navy-100/20 bg-white dark:bg-navy-400">
              <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400">Date</th>
              <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400">Type</th>
              <th className="px-4 py-3 text-left text-slate-500 dark:text-slate-400">Number</th>
              <th className="px-4 py-3 text-right text-slate-500 dark:text-slate-400">Debit</th>
              <th className="px-4 py-3 text-right text-slate-500 dark:text-slate-400">Credit</th>
              <th className="px-4 py-3 text-right text-slate-500 dark:text-slate-400">Balance</th>
            </tr>
          </thead>
          <tbody>
            {(d?.rows ?? []).map((r, i) => (
              <tr key={i} className="border-b border-slate-200 dark:border-navy-100/10">
        ...
        <div className="bg-slate-100 dark:bg-navy-100/20 px-4 py-2 text-gold font-number">Closing balance: <AmountDisplay amount={d?.closing_balance} /></div>
      </div>
    </ReportWrapper>
  );
}
