  ...
  return (
    <ReportWrapper title="Balance Sheet" mode="single" singleDate={asOn} onSingleDateChange={setAsOn} onExportExcel={() => {}} onExportPdf={() => {}} onPrint={() => window.print()}>
      <div className="flex gap-2 mb-4">
        <button type="button" onClick={() => setFormatToggle("tally")} className={`rounded-lg px-3 py-1.5 text-sm ${formatToggle === "tally" ? "bg-gold text-navy" : "bg-navy-100/30 text-slate-400"}`}>Tally format</button>
        <button type="button" onClick={() => setFormatToggle("icai")} className={`rounded-lg px-3 py-1.5 text-sm ${formatToggle === "icai" ? "bg-gold text-navy" : "bg-navy-100/30 text-slate-400"}`}>ICAI format</button>
      </div>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="rounded-xl border border-navy-100/20 overflow-hidden">
          <div className="bg-navy-100/20 px-4 py-2 font-semibold text-gold">Assets</div>
        ...
        <div className="rounded-xl border border-navy-100/20 overflow-hidden">
          <div className="bg-navy-100/20 px-4 py-2 font-semibold text-gold">Liabilities</div>
          <ul className="divide-y divide-navy-100/10">
            {liabilities.map((l, i) => (
